...
export default function BagApp({ initial, initialResult }: Props) {
  ...
  // nudge the freshly revealed card into view if it landed below the fold
  // (gentle — never a full-screen takeover; the form stays above it)
  useEffect(() => {
    if (result && !reduce) {
  ...
  return (
    <main className={`flex min-h-dvh flex-col px-4 ${result || error ? "py-12" : "justify-center py-10"}`}>
